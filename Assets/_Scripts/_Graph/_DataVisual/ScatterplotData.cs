...
public class ScatterplotData : MonoBehaviour
{
    ...
    private void RewriteData(int xData, int yData)
    {
        ...
        foreach (point p in points)
        {
            ...
            if (colors.isOn)
            {
                if ((p.crop == "Corn" || p.crop == "Corns") && (cropType.value == 0 || cropType.value == 1))
                {
                    PlotPoint("Corn", p.Get(xData), p.Get(yData));
                    Debug.Log("Plot corn");
                }
                ...
                else if ((p.crop == "Tomato" || p.crop == "Tomatoes") && (cropType.value == 0 || cropType.value == 3))
                {
                    PlotPoint("Mystery", p.Get(xData), p.Get(yData));
